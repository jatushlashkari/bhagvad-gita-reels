...
import type { AssetInfo, Backend, StateSummary } from './backend.ts';
...
async function saveImage(name: string, data: Buffer): Promise<AssetInfo> {
  ...
  const jpeg = await sharp(data)
    ...
    .jpeg({ quality: 82 })
    .toBuffer();
  await writeFile(join(REPO_ROOT, 'public/assets/images', file), jpeg);
  const manifest = await readManifest();
...
export const localBackend: Backend = { repoRoot, listAssets, saveImage, getState, getVerse, sync };
