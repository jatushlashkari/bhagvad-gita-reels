...
export interface Backend {
   ...
   *  and returns its combined stdout/stderr as a live stream ending in `EXIT <code>`, or the
   *  string 'locked' if a render is already in progress (see out/.render-lock, §5). */
  generate(ref: string, background?: string): ReadableStream<Uint8Array> | 'locked';
}
