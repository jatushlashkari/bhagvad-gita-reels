...
import { Toggle, ghostButtonClass, iconButtonClass, labelClass, selectClass } from '../studio/ui.tsx';
    ...
    <div>
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div className="flex flex-wrap items-center gap-4 pb-2">
          ...
          <span data-testid="quote-count" className="text-xs tabular-nums text-[#a89f8d]">
      ...
      {filtered.length > MAX_ROWS && (
        <p className="mt-3 text-xs text-[#e8c874]/70">showing first {MAX_ROWS} — narrow the search</p>
      )}
            ...
            {shown.map((r) => (
              <Fragment key={r.ref}>
                <tr className="border-b border-white/5">
                  <td className={`${cellClass} whitespace-nowrap font-mono text-xs text-[#e8c874]`}>
                    गीता {r.chapter}.{r.verse}
                  </td>
                  <td className={`${cellClass} text-[#f5efe0]`}>{r.hook}</td>
                  <td className={`${cellClass} whitespace-nowrap text-[#a89f8d]`}>
                    <span className="tabular-nums">{r.beats.length}</span>
                    <span className={r.curated ? ' text-[#e8c874]' : ''}>{r.curated ? ' curated' : ' auto'}</span>
                    <button
                      type="button"
                  ...
                  <td className={cellClass}>
                    <button
                      ...
                      className={`rounded px-1 text-base leading-none transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#e8c874]/60 ${
                        r.favorite ? 'opacity-100' : 'opacity-25 hover:opacity-60'
                      }`}
                  ...
                  <td className={`${cellClass} text-xs text-[#a89f8d]`}>
                  ...
                  <td className={cellClass}>
                    <Link
                      ...
                      className="text-xs text-[#e8c874] transition-colors hover:underline"
                    >
                      Studio
                    </Link>
                  </td>
                  ...
                  <tr className="border-b border-white/5 bg-[#0d0817]/60">
                    <td className={cellClass} colSpan={6}>
                      <ul className="space-y-2">
                        {draft.map((b, i) => (
                          <li key={i} className="flex items-center gap-2">
                            <span className="w-3 shrink-0 text-right text-[10px] tabular-nums text-[#a89f8d]">
                              {i + 1}
                            </span>
                            <input
                              aria-label={`beat ${i + 1}`}
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-3">
                        ...
                        {draftProblem && <span className="text-xs text-red-400">{draftProblem}</span>}
                        {!draftProblem && editError && <span className="text-xs text-red-400">{editError}</span>}
                      </div>
                    </td>
                  </tr>
                )}
      ...
      {filtered.length === 0 && <p className="mt-3 text-sm text-[#a89f8d]">nothing matches those filters</p>}
    </div>
  );
}
